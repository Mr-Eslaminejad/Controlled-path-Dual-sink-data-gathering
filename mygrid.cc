...
*/
void MyGridAgent::find_nearest_sink(const char* packet_type, float xpos, float ypos, float x_event, float y_event)
{
 //Finding the nearest sink

        if ((sink2_x == x_event) && (sink2_y == y_event)) {
           	float d1, d2;
	   	d1 = sqrt(((xpos-sink1_x)*(xpos-sink1_x))+((ypos-sink1_y)*(ypos-sink1_y)));
	   	d2 = sqrt(((x_event-sink1_x)*(x_event-sink1_x))+((y_event-sink1_y)*(y_event-sink1_y)));
           	printf("Source distance to SS= %f and Event distance to SS= %f",d1,d2);
		if (d1 > d2) {
			nearsink_id = 1;						
		} //else if ((d1 == d2) && (sink1 residual energy)) {
		else {
			nearsink_id = 0;
		}
	} else {
...
//                        float energy = iEnergy;
        ...
        else if (argc == 7) {                            //send data message
		if (strcmp(argv[1], "send") == 0) {
                        float x_s = atof(argv[3]);
                        float y_s = atof(argv[4]);
                        float x_event = atof(argv[5]);
                        float y_event = atof(argv[6]);
                        find_nearest_sink(argv[2],x_s,y_s,x_event,y_event);
			//mysend(0,0,0,argv[2],x_s,y_s,0,0,0,0);
			return (TCL_OK);
		}
	}
        else if (argc == 4) {                            //sink position update
		if (strcmp(argv[1], "sink1_update_pos") == 0) {      //static sink (SS)
                        sink1_x = atof(argv[2]);
                        sink1_y = atof(argv[3]);
			return (TCL_OK);
		}
		else if (strcmp(argv[1], "sink2_update_pos") == 0) { //mobile sink (MS)
                        sink2_x = atof(argv[2]);
                        sink2_y = atof(argv[3]);
			return (TCL_OK);
		}
	}
        else if (argc == 5) {                            //total results at the end of simulation
