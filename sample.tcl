# for each scenario we should set V (sink speed), R (radio range), R_event, ST, val(x), val(y), nn, initenergy, num_row, nop, src and sink positions and sink movement
...
$node_(0) set X_ $midx                    ;#Static Sink (SS) at the center point of the network
$node_(0) set Y_ $midy 
$node_(0) set Z_ 0.0 

set ms_x [expr {floor(rand()* $fullx)}]   ;#Random variable as MS X position between 0 to fullx-1 
set ms_y [expr {floor(rand()* $fully)}]   ;#Random variable as MS Y position between 0 to fully-1 

$node_(1) set X_ $ms_x                    ;#Mobile Sink (MS) at the random point of the network
$node_(1) set Y_ $ms_y 
$node_(1) set Z_ 0.0 

set postable(0,0) $midx 
set postable(1,0) $midy
set postable(0,1) $ms_x 
set postable(1,1) $ms_y
...
set R_event 75							;#the radius in which the event occured
set total_event_nodes 0	;#sum of all event nodes during the simulation used for calculating total number of packets
set period 6.000
while {$firetime < $ST} {
	set rnd_f [expr {2 + floor(rand()* $n_sensor)}]         ;#Random variable as event point between 2 to 50 
	set event [expr int($rnd_f)]				;#define a node as event point
	set event_nodes 0					;#number of event nodes in a point 
	$ns_ at $firetime "movement $firetime $postable(0,$event) $postable(1,$event)"	;# sink2 (MS) movement
	for {set i 2} {$i < $sensor } {incr i} {
	...
	for {set i 0} {$i < $nop } {incr i} {
		...
		for {set j 0} {$j < $event_nodes } {incr j} {
			...
			$ns_ at $multifire "$p($src($j)) send DATA $xpos $ypos $event_x $event_y" 
...
proc movement {MS_time MS_x MS_y} {
global ns_ node_
set V 20.0
$ns_ at $MS_time "$node_(1) setdest $MS_x $MS_y $V"            ;# sink2 (MS) movement
}
